...
RESULTS_FILE = Path("exp1_results.jsonl")
HISTORY_DIR = Path("exp1_history_logs")
CAPSULE_FILE = Path("exp1_checkpoint_capsules.json")
...
class ReplayAgent:
    """Baseline agent: no persistence, full replay."""
...
class CheckpointAgent:
    """Checkpoint agent with disk-persisted capsules."""
...
async def run():
    ...
    print("\n=== EXPERIMENT: REPLAY vs CHECKPOINT (10 tickers, disk persistence) ===\n")
    ...
    for i in range(1, RESTARTS + 1):
        ...
        async with stdio_client(SERVER_PARAMS) as (r, w):
            async with ClientSession(r, w) as session:
                ...
                replay = ReplayAgent()        # loses memory
                checkpoint = CheckpointAgent()  # reloads capsules from disk
